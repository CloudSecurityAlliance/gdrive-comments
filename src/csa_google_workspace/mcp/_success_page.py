...
Served by the loopback listener that receives Google's redirect. Self-contained apart from
the CSA TypeKit stylesheet: the page must still render correctly offline or if TypeKit is
blocked, so every font has a real fallback stack.

The CSA logo is inlined verbatim from the official asset and deliberately NOT recolored —
brand rules forbid it. Note the asset carries the superseded orange (#F98526); the current
palette value is #FF7A00. This page therefore uses no orange of its own, so the two never
appear side by side.
"""
...
<title>Authorized — CSA Google Workspace</title>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  <main class="card">
    ...
    <p>The Google Workspace MCP server can now read and triage comments on your
       Docs, Sheets, and Slides &mdash; as you, with your own access.</p>
