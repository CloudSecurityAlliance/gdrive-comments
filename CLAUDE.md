...
**Phase 2 — the built-in MCP server (`csa_google_workspace.mcp`) — shipped in v0.2.2.** A local stdio server: **51 tools** as of v0.39.0 (it shipped with nine), each with structured output and read-only/destructive annotations, per-user OAuth, an `[mcp]` optional extra, and the `csa-google-workspace-mcp` console script. Spec: [`docs/superpowers/specs/2026-07-23-mcp-server-design.md`](docs/superpowers/specs/2026-07-23-mcp-server-design.md) — read it before touching the module; its architecture deliberately **mirrors the library's own composition** (`create_server(get_workspace)` parallels `Workspace`; per-axis `register_*` tool producers parallel `CommentsMixin` / `documents/`).
...
- **`THREAT_MODEL.md`** (repo root) — the **living threat register**: 36 enumerated threats, each with actor, surface, impact × likelihood, a current `status` and its evidence. Adopted 2026-08-28 from audit `2026-08-27-01` (#197); its threat text is as that audit wrote it, **§0** accounts for every status that has moved since, and **§0b** records every threat ADDED since — with a source, because a threat absent from the frozen snapshot is either a genuine finding or a copy-paste and only a written provenance tells them apart. T36 (access-request text, from #273) is the first. An audit **proposes** changes here by filing an issue, never by editing the file. `tests/test_threat_model.py` asserts §0 explains every difference from the audit's frozen snapshot — so a status cannot quietly improve.
