...
#   ./Run-full-test-suite.sh --debug             # trace every command to a log
#   ./Run-full-test-suite.sh --check --report    # write a shareable report even on success
...
# $HOME -> ~ for anything bound for a public tracker, and ONLY at a path boundary.
# A blind `s|$HOME|~|` is wrong on a machine whose paths CONTAIN $HOME: this repo lives at
# /Volumes/MacMiniData/Users/kurt/..., which the naive version turned into
# "/Volumes/MacMiniData~/GitHub/...". Anchored to start-of-line or a space/=/backtick.
# Two passes. The first rewrites the real home to ~. The second collapses ANY /Users/<name>,
# which catches a home-equivalent path the first cannot recognise — this checkout lives at
# /Volumes/MacMiniData/Users/kurt/..., outside $HOME but still carrying the username.
redact_home() {
    sed "s|\(^\|[ =\`]\)$HOME|\1~|g" | sed -E "s|/Users/[^/ ]+|/Users/<user>|g"
}

# The report a human attaches to an issue. Produced on demand (--report) as well as after a
# failure, because the case that motivated it had nothing "fail": a fresh machine where
# --check simply showed missing prerequisites, and the reporter had to hand-write the analysis
# by reading the scripts (#440).
write_report() {
    mkdir -p "$RIG_DIR"
    REPORT="$RIG_DIR/issue-report.md"
    {
        ...
        if (( ${#RESULTS[@]} )); then
            ...
            echo "- no layers ran (\`--check\`, or stopped before the layers)"
        fi
        echo
        ...
        echo "tested    : ${VERSION:-(not resolved)}"
        # $HOME -> ~ : `_environment` reports no filesystem paths precisely because this text
        # is written for a PUBLIC tracker, and an absolute path carries a username.
        [[ -n "$IMPORT_CHECK" ]] && echo "$IMPORT_CHECK" | grep -E '^(path|version)=' | redact_home
        echo '```'
        echo
        echo "### Prerequisites"
        echo
        echo '```'
        row() { printf '%-14s: %s\n' "$1" "$2"; }
        row os "$(uname -s) $(sw_vers -productVersion 2>/dev/null)"
        if [[ -n "$PYTHON" ]]; then
            row python "$("$PYTHON" -c 'import platform; print(platform.python_version())' 2>/dev/null) ($PYTHON)"
        else
            row python "NONE FOUND >= 3.10"
        fi
        for t in git gh pipx claude playwright; do
            command -v "$t" &>/dev/null && row "$t" present || row "$t" MISSING
        done
        row plugin "$(command -v csa-google-workspace-mcp &>/dev/null \
            && csa-google-workspace-mcp --version 2>&1 | head -1 || echo 'not installed')"
        row "client secret" "$([[ -f "$CLIENT_SECRETS" ]] && echo present || echo MISSING)"
        row "token rw" "$([[ -f "$TOKEN_RW" ]] && echo present || echo missing)"
        row "token ro" "$([[ -f "$TOKEN_RO" ]] && echo present || echo 'missing (L2-RO skips)')"
        echo '```'
        echo
        echo "### Environment"
        echo
        echo '```'
        if [[ -n "$PROBE_PY" ]]; then
            echo "# reported by $(echo "$PROBE_PY" | redact_home) — this describes the"
            echo "# interpreter the report was GENERATED with, which is not necessarily the"
            echo "# artifact under test above."
            "$PROBE_PY" -c \
                'from csa_google_workspace._environment import describe_environment
print(describe_environment().as_markdown())' 2>/dev/null
        else
            echo "(package not installed here, so no environment block)"
        fi
        echo '```'
        echo
        echo "### Reproduce"
        echo
        echo '```bash'
        echo "./Run-full-test-suite.sh --version $VERSION_SPEC --layers $LAYERS$($CHECK_ONLY && echo ' --check')"
        echo '```'
        echo
        [[ -f "$RUN_LOG" ]] && \
            echo "Full log on the machine that ran it: \`$(echo "$RUN_LOG" | redact_home)\`"
        [[ -n "$DEBUG_LOG" ]] && \
            echo "Command trace: \`$(echo "$DEBUG_LOG" | redact_home)\`"
        echo
        echo "_Generated by \`Run-full-test-suite.sh\`. It carries no Drive file ids, document"
        echo "titles, comment text or filesystem paths outside \`~\` — do not paste the raw run"
        echo "log in beside it, which can._"
    } > "$REPORT"
}


# --- Parse arguments ---
# Kept before the loop shifts them away: --debug re-execs this script, and `"$@"` after
# parsing is EMPTY. The first version of that re-exec therefore ran a FULL run when asked for
# --check, creating the venv and worktree that --check exists not to create. Caught because
# the directories were there afterwards.
ORIGINAL_ARGS=("$@")
...
# --debug STOPS SWALLOWING OUTPUT, and keeps a copy.
#
# Not a shell trace. The first attempt used `set -x` with BASH_XTRACEFD, which silently did
# nothing — that variable is bash-only and this is zsh. More to the point it was the wrong
# feature: what went wrong in #440 was that the pip and pipx output was thrown away by
# `--quiet` and `>/dev/null`, so the one thing worth reading was the one thing discarded.
#
# So --debug makes the installs verbose and tees the whole run to a file. Re-exec once, with
# a guard variable, so the tee covers everything including the prerequisite checks above.
DEBUG_LOG=""
if $DEBUG; then
    DEBUG_LOG="$RIG_DIR/debug.log"
    if [[ -z "$RIG_DEBUG_CHILD" ]]; then
        mkdir -p "$RIG_DIR"
        RIG_DEBUG_CHILD=1 "$0" "${ORIGINAL_ARGS[@]}" 2>&1 | tee "$DEBUG_LOG"
        exit ${pipestatus[1]}
    fi
fi

# Installs are quiet by default because a successful one has nothing to say; under --debug
# every line is kept, which is the case where it matters.
if $DEBUG; then PIP_Q=(); PIPX_OUT=/dev/stdout
else                PIP_Q=(--quiet); PIPX_OUT=/dev/null
fi
...
if $CHECK_ONLY; then
    ...
    "$RIG_VENV/bin/pip" install "${PIP_Q[@]}" --upgrade pip

    if [[ "$VERSION_SPEC" == "tree" ]]; then
        "$RIG_VENV/bin/pip" install "${PIP_Q[@]}" -e "${SCRIPT_DIR}[dev,mcp]"
        ...
        "$RIG_VENV/bin/pip" install "${PIP_Q[@]}" --no-cache-dir "${PKG}[mcp,dev]==$VERSION"
    ...
    if command -v pipx &>/dev/null; then
        if [[ "$VERSION_SPEC" == "tree" ]]; then
            pipx install --force "${SCRIPT_DIR}[mcp]" >"$PIPX_OUT" 2>&1 && pass "pipx: installed from the checkout" \
                || warn "pipx install from the checkout failed"
        else
            pipx install --force "${PKG}[mcp]==$VERSION" >"$PIPX_OUT" 2>&1 && pass "pipx: $PKG==$VERSION" \
...
if $CHECK_ONLY; then
    if $WANT_REPORT; then
        ...
        info "attach it to an issue — it names the versions and what is missing, and carries"
        info "no file ids, document text or paths outside ~"
    fi
