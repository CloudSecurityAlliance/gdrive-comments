...
| 2026-08-28 | **Defaults are open; the documentation's job is how to narrow.** All capabilities on, both allowlists `*` | the README already told operators to set `READ="*"`; `THREAT_MODEL.md` §1 already named Drive as the primary layer and this one as "deliberately narrow, not the primary layer" | #82's fail-closed posture (the `_GATES` fail-closed invariant is unaffected and stays) |
